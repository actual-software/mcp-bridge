...
func TestTCPHealthServer_Metrics(t *testing.T) {
	...
	require.InDelta(t, float64(0), errorCount, 0.01, "Expected no health receive errors")

	errorCount = testMetrics.GetTCPProtocolErrorCount("health_send_error")
	require.InDelta(t, float64(0), errorCount, 0.01, "Expected no health send errors")

	errorCount = testMetrics.GetTCPProtocolErrorCount("health_flush_error")
	require.InDelta(t, float64(0), errorCount, 0.01, "Expected no health flush errors")
}
